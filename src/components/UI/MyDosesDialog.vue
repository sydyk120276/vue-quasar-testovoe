<template lang="">
  <div
    ...
    v-show="sessionAfterAppointmentsStore.dialogVisibleDoses"
    @click="sessionAfterAppointmentsStore.closeShowDialogDoses"
  >
    <div
      class="m-auto bg-white rounded-[2px] min-w-[275px] p-[20px]"
      @click.stop
    >
        <div v-for="(mask) in sessionAfterAppointmentsStore.typeDoses" :key="mask.id" class="w-[200px] flex justify-between border border-gray-300">
          <button
            class="w-full justify-center flex items-center"
            @click="sessionAfterAppointmentsStore.onClickDoses(mask.title)"
          >
          {{ mask.title }}
          </button>
        </div>
