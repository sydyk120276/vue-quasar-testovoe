<template lang="">
  <div
    ...
    v-show="catheterStoreType.dialogVisibleCatheterType"
    @click="catheterStoreType.closeShowDialogCatheterType"
  >
    <div
    ...
    >
      ...
      <div class="flex flex-col">
        <div class="w-[244px] flex justify-between pb-[10px]">
          <input
            :value="catheterStoreType.searchValueCatheterType"
            @change="catheterStoreType.catheterFilterCatheterType"
            type="text"
        ...
        <div class="w-[244px] flex justify-between pb-[10px]">
          <input
            :value="catheterStoreType.valueInputCatheterType"
            @input="catheterStoreType.handleInputCatheterType"
            @keypress.enter="catheterStoreType.addRecordCatheterType"
            type="text"
            class="outline-none border border-gray-300 text-[8px] font-semibold w-[215px] h-[20px] flex items-center"
          ...
          <button
            class="border border-gray-300 w-[23px] h-[20px] justify-center flex items-center"
            @click="catheterStoreType.addRecordCatheterType"
          >
           <RecordIcon />
          </button>
        </div>
        <div v-for="(mask) in catheterStoreType.needToListCatheterType" :key="mask.id" class="w-[244px] flex justify-between relative border border-gray-300">
          <span class="text-[8px] font-semibold flex items-center">{{ mask.title }}</span>
          <button class="absolute right-[25px] top-[3px] "
          @click="catheterStoreType.removeCatheterType(index)">
            <DeleteIcon />
          </button>
          <button
            class="w-[23px] h-[20px] justify-center flex items-center border-l"
            @click="catheterStoreType.onClickCatheterType(mask.title)"
          >
          <StoreValueIcon />
          </button>
        </div>
        <div v-for="(mask) in catheterStoreType.searchCatheterType" :key="mask.id" class="w-[244px] flex justify-between relative border border-gray-300">
          <span class="text-[8px] font-semibold flex items-center">{{ mask.title }}</span>
          <button class="absolute right-[25px] top-[3px] "
          @click="catheterStoreType.removeMaskCatheterType(index)"
          >
          <DeleteIcon />
          </button>
          <button
