<template lang="">
  <div
    ...
    v-show="sessionAppointmentsStore.dialogVisibleCatheterType"
    @click="sessionAppointmentsStore.closeShowDialogCatheterType"
  >
    <div
      class="m-auto bg-white rounded-[2px] min-w-[275px] p-[20px]"
      @click.stop
    >
        <div v-for="(mask) in sessionAppointmentsStore.searchCatheterType" :key="mask.id" class="w-[200px] flex justify-between border border-gray-300">
          <button
            class="w-full justify-center flex items-center"
            @click="sessionAppointmentsStore.onClickCatheterType(mask.title)"
          >
          {{ mask.title }}
          </button>
        </div>
